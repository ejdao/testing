...
import { ChevronUp, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, FileText, ClipboardCheck, BarChart3, ShoppingCart, FilePlus, CalendarClock, Receipt } from 'lucide-react'
...
interface RequestsTableProps {
  ...
  onAccountPayment: (request: PurchaseRequest) => void
}
...
export function RequestsTable({ requests, onViewProducts, onAddQuotation, onReviewRequest, onCompareQuotations, onApprovePurchase, onCreatePurchaseOrder, onSchedulePayment, onAccountPayment }: RequestsTableProps) {
                  ...
                  <tr key={request.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {(request.estado === 'PENDIENTE POR CONTABILIZAR OC' || 
                          request.estado === 'PENDIENTE POR CONTABILIZAR') && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => onAccountPayment(request)}
                            className="h-8 gap-1.5 border-cyan-300 text-cyan-700 hover:bg-cyan-50"
                            title="Contabilizar"
                          >
                            <Receipt className="h-3.5 w-3.5" />
                            <span className="sr-only sm:not-sr-only sm:inline">Contabilizar</span>
                          </Button>
                        )}
                      </div>
                    </td>
